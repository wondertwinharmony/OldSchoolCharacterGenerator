export const weaponQualities = `<strong>Blunt:</strong> May be used by clerics.  

<strong>Brace:</strong> When braced against the ground, doubles damage against charging monsters.

<strong>Charge:</strong> On horseback, moving at least 60’ in a round and attacking doubles any damage done with a successful hit.  

<strong>Melee:</strong> Close quarters weapon (5’ or less).  

<strong>Missile:</strong> Thrown or fired weapon (greater than 5’ distance). The distances for short (+1 to hit), medium, and long (-1 to hit) range are shown in parentheses.    

<strong>Slow:</strong> The character acts last in each combat round (see Basic Combat Procedure in B/X Essentials: Core Rules).  

<strong>Splash weapon:</strong> On a successful missile attack, the container smashes and the target is doused in the liquid. Damage is inflicted over two rounds, as the liquid drips off.   

<strong>Two-handed:</strong> The weapon requires both hands; the character cannot use a shield.`;

export const weaponQualitiesArr = [
    "Blunt: May be used by clerics.",  
    
    "Brace: When braced against the ground, doubles damage against charging monsters.",   
    
    "Charge: On horseback, moving at least 60’ in a round and attacking doubles any damage done with a successful hit.",  
    
    "Melee: Close quarters weapon (5’ or less).",  
    
    "Missile: Thrown or fired weapon (greater than 5’ distance). The distances for short (1 to hit), medium, and long (-1 to hit) range are shown in parentheses.",  
    
    "Reload (optional rule): Requires a round to reload between shots; can only be fired every second round.",  
    
    "Slow: The character acts last in each combat round (see Basic Combat Procedure in B/X Essentials: Core Rules).",  
    
    "Splash weapon: On a successful missile attack, the container smashes and the target is doused in the liquid. Damage is inflicted over two rounds, as the liquid drips off.",   
    
    "Two-handed: The weapon requires both hands; the character cannot use a shield."
    ]